...
Custom `Combobox` (not native `<select>`) with internal search, 5-row scroll, and a disabled state for "En desarrollo" mutuas (`COMING_SOON_MUTUAS`). Hoy operativas las 13 de `AVAILABLE_MUTUAS`; `COMING_SOON_MUTUAS` = `["Generali", "Sanitas"]` (ambas requieren `SANITAS_APIKEY`, que no está disponible: Sanitas usa esa key y Generali delega en Sanitas — vuelven a `AVAILABLE_MUTUAS` en cuanto se configure). El formulario completo se sirve en el hero de **`app/page.tsx`** (la home); el chatbot queda accesible vía un FAB anclado abajo-derecha (ver «Chatbot asistente»). Las páginas programáticas (`/cuadro-medico/...`, `/medicos/...`) lo embeben con `<SearchForm compact />`.
