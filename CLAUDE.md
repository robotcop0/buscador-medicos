...
La conversación persiste en `localStorage` (clave gestionada por `ChatWidget`), así que cerrar el panel o navegar entre `/` y `/resultados` conserva el hilo. El `ChatWidget` acepta `variant?: "hero" | "panel"`; hoy solo se usa `"panel"` (el `"hero"` queda como default retrocompatible por si en el futuro se vuelve a montar inline).

La ruta `/manual` (que en su día albergaba el buscador standalone) **se ha eliminado**: la home ya cubre ese caso de uso.

1. **Route `app/api/chat/route.ts`** (`POST`, `runtime = "nodejs"`, `dynamic = "force-dynamic"`): bucle agéntico **manual** sobre `@anthropic-ai/sdk` (modelo `claude-haiku-4-5`, `max_tokens` 2048, sin `thinking`/`effort` — Haiku no los acepta). `system` cacheado con `cache_control: ephemeral` — y el system prompt está deliberadamente "largo" (catálogos + guía + glosario síntoma→especialidad + notas por mutua + ejemplos) para que el prefijo cacheable (tools + system) supere el **mínimo de ~4096 tokens que Haiku 4.5 exige para cachear**; si se recorta por debajo de eso el caching deja de funcionar sin error. En dev se loguea `usage` (`in/cacheWrite/cacheRead/out`) tras cada llamada para verificarlo. Recibe `{ messages: ChatMessage[] }`, devuelve `ChatApiResponse`. Rate-limit best-effort en memoria por IP (20 msgs / 10 min) y tope de longitud de conversación (24 turnos). Errores de la API se mapean con las clases tipadas (`Anthropic.RateLimitError`, `Anthropic.APIError`).
...
3. **System prompt** (`lib/chatbot/system-prompt.ts`) + **catálogo** (`lib/chatbot/catalog.ts`, espejo de las listas de `SearchForm`): reglas de "nunca buscar sin tener los 4: mutua + especialidad + ubicación + radio" (el radio se pregunta SIEMPRE antes de buscar, aunque el resto venga de golpe), "pedir lo que falta de uno en uno", "para el radio ofrecer siempre 2/10/25/50/100 km + Toda la provincia + custom", "no inventar médicos", formato Markdown del resultado, y disclaimer de que no reserva citas.
4. **Cliente (`components/ChatWidget.tsx`)**: client component montable en variante "panel" (también soporta "hero" para retrocompatibilidad). Panel con cabecera "Asistente · Buscador de Médicos" + puntito verde, avatar 🩺 en las burbujas, indicador de "pensando" con tres puntos animados, y **chips de sugerencia** (`SUGGESTIONS`) cuando la conversación está vacía (solo el saludo). Mantiene dos arrays — `apiMessages` (lo exacto que va/viene de `/api/chat`) y `display` (lo que se renderiza) — persistidos en `localStorage` (`buscador-chatbot-v1`; botón "Reiniciar"). Renderiza el Markdown del asistente con `react-markdown` + `remark-gfm`. Mensaje de bienvenida fijo (no consume API).
...
Especialidad uses **accent-insensitive partial match** so the UI dropdown ("Cardiología") matches subspecialties ("Cardiología Infantil", "Cirugía General Y Del Aparato Digestivo", etc.). When `maxKm` is absent, falls back to province match (`cp.slice(0,2)`). Sort (`lib/ratings-sort.ts`): valorados primero por **score bayesiano↓** (fórmula tipo IMDb: `v/(v+m)·R + m/(v+m)·C`, donde `C` = media de notas del listado y `m` = mediana de nº de reseñas del listado, acotada a [8, 150] — adaptativa: en listados de centros pequeños basta poco volumen para destacar, en listados de hospitales grandes hace falta más), empate por distancia↑ y luego numReviews↓; no valorados después por distancia↑/nombre. Esto evita que un 5,0 con 1 reseña adelante a un 4,9 con miles. El score se guarda en `Doctor.rankScore` y `DoctorCard` muestra **ese score** (número + color verde/ámbar/rojo) en el pill, no la nota cruda, para que lo que se ve coincida con el orden del listado (el nº de reseñas debajo da el contexto).
...
Custom `Combobox` (not native `<select>`) with internal search, 5-row scroll, and a disabled state (badge "En mantenimiento") for `COMING_SOON_MUTUAS`. Operativas las 15 de `AVAILABLE_MUTUAS` (Sanitas/Generali requieren `SANITAS_APIKEY` en el env del servidor). `COMING_SOON_MUTUAS` = aseguradoras españolas aún no integradas (`Aegon`, `HNA`, `Nueva Mutua Sanitaria`, `PSN`, `Assistència Sanitària`, `MGC Mútua`): aparecen deshabilitadas hasta investigar su cuadro médico (ver «Methodology — adding a new mutua») y moverlas a `AVAILABLE_MUTUAS`. El formulario completo se sirve en el hero de **`app/page.tsx`** (la home); el chatbot queda accesible vía un FAB anclado abajo-derecha (ver «Chatbot asistente»). Las páginas programáticas (`/cuadro-medico/...`, `/medicos/...`) lo embeben con `<SearchForm compact />`.

### Blog (`app/blog/`)
Sección editorial estática. `lib/blog.ts` es el registro: `BLOG_POSTS` (metadata `slug/title/description/excerpt/date/readingMinutes`) + `Body`, un componente por artículo en `app/blog/_posts/<slug>.tsx` (importado estáticamente al final de `lib/blog.ts`). `app/blog/page.tsx` = índice (lista ordenada por fecha↓). `app/blog/[slug]/page.tsx` = artículo, con `generateStaticParams`, `generateMetadata` y JSON-LD `BlogPosting`. Las dos páginas usan la estética del resto del sitio (`<SiteFooter>`, breadcrumb, eyebrow). Enlace discreto a `/blog` en el `<nav>` del eyebrow de la home. `app/sitemap.ts` añade `/blog` + las URLs de los artículos. Sin MDX, sin RSS, sin categorías — añadir un artículo = crear el `.tsx` en `_posts/` + una entrada en `BLOG_POSTS`. **Nota Next 16**: `params` en `[slug]/page.tsx` es `Promise` — hay que `await`.
