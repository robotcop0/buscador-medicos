...
1. **Verificación del techo (de-risk, primero):** spot-check de ~30 médicos sin nota de municipios pequeños — buscarlos a mano en Doctoralia y Google. Resultado decide la mezcla de palancas:
   ...
   **Verificación 2026-06-12:** 1/30 en Doctoralia sin scrapear, 28 Google (93%), 1 nowhere → **el gap NO es un problema de Doctoralia**: solo 1 de los 30 sin nota está en el índice de Doctoralia (y con rating=0). El 93% son hallados por el sidecar de Google Maps, pero casi todos son falsos positivos de proximidad (el sidecar devuelve el hospital/clínica más cercana, no al médico en sí — p.ej. "Recoletas Zamora" → "Hospital Recoletas Salud Zamora"). Esto confirma que: (a) ampliar el scrape de Doctoralia tiene rendimiento bajo para provincias pequeñas; (b) la **palanca principal es el batch de Google con fallback-centro** (paso 4+5 del plan), que capturará el rating del centro al que está adscrito el médico; (c) hay además ~4 nombres tipo "Recoletas Zamora" / "Recoletas Palencia" / "Megalab Zamora" que son centros no detectados por `CENTER_RE` — conviene ampliar esa regex para evitar que esos registros entren en el pool de "personas". En resumen: **priorizar `enrich-google-batch` + fallback-centro antes que ampliar el scrape de Doctoralia**.

   **Probe Doctoralia en vivo 2026-06-12 (T9 — `scripts/probe-doctoralia.ts`):** 24 médicos sin nota (12 urbanos + 12 rurales), búsqueda live por nombre en doctoralia.es, sin bloqueos anti-bot. Resultado bruto: urbano 6/12 on-Doctoralia (50% — 1 rated, 5 no-rating), rural 3/12 on-Doctoralia (25%). Tras revisar falsos positivos: 1 match rural es falso positivo por coincidencia de tokens comunes ("Francisco Martin"), y 2 rurales ("Recoletas Zamora", "Megalab Zamora") son centros no detectados por `CENTER_RE`. **Tasa corregida**: urbano ~50% on-Doctoralia (pero todos con 0 reseñas en Doctoralia — están en el índice pero no tienen rating); rural ~8-10% on-Doctoralia con rating real. Conclusión: (a) **Urbano**: muchos sí están en Doctoralia pero con 0 reseñas — ampliar el scrape capturaría perfiles pero sin nota útil; (b) **Rural**: la gran mayoría (>90%) no están en Doctoralia ni tienen nota propia → cola honesta o fallback-centro; (c) la fuente de ganancia sigue siendo el **batch de Google + fallback-centro**, no el scrape de Doctoralia; (d) hay que ampliar `CENTER_RE` para filtrar "Recoletas X" / "Megalab X" como centros antes de que entren en el pool de personas.
